"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var core_1 = require('@angular/core');
var product_type_model_1 = require('./../../_models/product-type.model');
var ProductTypeComponent = (function () {
    function ProductTypeComponent() {
        this.productTypes = [
            new product_type_model_1.ProductTypeModel(1, "Food"),
            new product_type_model_1.ProductTypeModel(2, "Cleaning Supplies"),
            new product_type_model_1.ProductTypeModel(3, "Technical equipment")
        ];
    }
    ProductTypeComponent.prototype.createNewProductType = function (event) {
        console.log('You will create new Product Type Classificator');
        event.preventDefault();
    };
    ProductTypeComponent = __decorate([
        core_1.Component({
            selector: 'product-type',
            templateUrl: './../app/classificators/product-type/product-type.component.html'
        }), 
        __metadata('design:paramtypes', [])
    ], ProductTypeComponent);
    return ProductTypeComponent;
}());
exports.ProductTypeComponent = ProductTypeComponent;
//# sourceMappingURL=product-type.component.js.map